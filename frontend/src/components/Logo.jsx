...
export const LogoMark = ({ className = "w-6 h-6", glow = false }) => {
  return (
    <svg 
      className={`${className} ${glow ? 'drop-shadow-[0_0_8px_rgba(37,99,235,0.2)]' : ''} transition-all duration-300`} 
      viewBox="0 0 100 100" 
      fill="none" 
...
export const Logo = ({ className = "flex items-center gap-2", lightMode = true }) => {
  return (
    <div className={className}>
      <LogoMark className="w-6 h-6 text-[#111827]" />
      <span 
        className={`font-display font-semibold text-[20px] tracking-tight transition-colors duration-300 ${
          lightMode ? 'text-[#111827]' : 'text-white'
        }`}
      >
        Shop<span className="text-[#2563EB] font-bold">EZ</span>
      </span>
    </div>
  );
};
