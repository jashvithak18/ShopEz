...
export default function ProductDetail() {
  ...
  if (loading) return <div className="max-w-7xl mx-auto px-6 py-32 mt-16"><DetailSkeleton /></div>;
  if (!product) return <div className="text-center py-32 font-display font-bold">Product not found.</div>;
  return (
    <div className="max-w-7xl mx-auto px-6 py-20 mt-16 space-y-24 min-h-screen">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
        <div className="space-y-6">
          <div className="aspect-square w-full rounded-[40px] overflow-hidden bg-brand-50 border border-black/[0.04] relative shadow-sm flex items-center justify-center p-8">
          ...
          <div className="grid grid-cols-4 gap-4">
            {product.images.map((img, i) => (
              <button 
                ...
                className={`aspect-square rounded-2xl overflow-hidden bg-brand-50 border p-2 flex items-center justify-center ${activeImage === img ? 'border-brand-500 ring-4 ring-brand-500/10' : 'border-black/[0.04]'} transition-all`}
              >
        ...
        <div className="space-y-10">
          <div className="space-y-4">
            <span className="text-[10px] uppercase font-bold tracking-widest text-[#2563EB] font-sans">
              {product.seller?.storeName || 'AeroTech Studio'}
            </span>
            
            <h1 className="font-display font-semibold text-4xl sm:text-[48px] text-[#111827] tracking-tight leading-tight">
              {product.name}
            </h1>
            
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-0.5 text-[#2563EB] text-xs font-semibold">
            ...
            <p className="font-display font-semibold text-3xl text-[#111827] pt-2">
              ₹{price}
            </p>
          </div>
          
          <p className="text-[15px] text-[#1A1A1A]/60 leading-relaxed font-sans font-light border-t border-black/5 pt-8">
            {product.description}
          </p>
          {product.variants && product.variants.length > 0 && (
            <div className="space-y-4 border-t border-black/5 pt-8">
              <h4 className="font-display font-bold text-[10px] uppercase tracking-wider text-apple-dark/45">Configure Specifications</h4>
                  ...
                  <button
                    ...
                    className={`px-4 py-3 rounded-2xl border text-xs font-semibold transition-all ${
                      selectedColor === v.color && selectedSize === v.size
                        ? 'border-apple-dark bg-apple-dark text-white shadow-md'
                        : 'border-black/5 text-apple-dark/70 hover:bg-black/5'
                    }`}
                  >
          ...
          <div className="flex items-center gap-4 border-t border-black/5 pt-8">
            <div className="flex items-center border border-black/5 rounded-2xl bg-white p-1 shadow-sm">
            ...
            <button 
              ...
              className="flex-grow flex items-center justify-center gap-2.5 py-4 rounded-full bg-[#111827] hover:bg-[#2563EB] text-white text-[15px] font-sans font-medium transition-all duration-500 shadow-md cursor-pointer hover:scale-[1.01]"
            >
            ...
            <button 
              ...
              className="p-4 rounded-full border border-black/5 text-[#1A1A1A]/50 hover:text-red-500 hover:border-red-100 transition-all bg-white shadow-sm cursor-pointer"
            >
              <Heart className="w-4.5 h-4.5" />
            </button>
          </div>
          <div className="grid grid-cols-3 gap-6 border-t border-black/5 pt-8 text-[9px] font-bold uppercase tracking-wider text-apple-dark/45">
      ...
      <div className="border-t border-black/5 pt-16 space-y-6">
        <h3 className="font-display font-black text-xl text-apple-dark font-accent">Specifications</h3>
        <div className="max-w-xl bg-white border border-black/5 rounded-[32px] overflow-hidden shadow-sm">
      ...
      {related.length > 0 && (
        <div className="border-t border-black/5 pt-16 space-y-8">
          <h3 className="font-display font-black text-xl text-apple-dark font-accent">Frequently Bought Together</h3>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {related.map(prod => (
              <Link to={`/product/${prod._id}`} key={prod._id} className="bg-white p-5 rounded-3xl border border-black/5 shadow-sm hover:shadow-md transition-all group">
      ...
      <div className="border-t border-black/5 pt-16 grid grid-cols-1 lg:grid-cols-3 gap-16">
        <div className="lg:col-span-2 space-y-8">
          <h3 className="font-display font-black text-xl text-apple-dark font-accent">Reviews</h3>
          <div className="space-y-6">
            {reviews.length > 0 ? (
              reviews.map((rev, i) => (
        ...
        <div className="p-8 rounded-[32px] bg-white border border-black/5 shadow-md space-y-6 h-fit">
          ...
          <form onSubmit={handleReviewSubmit} className="space-y-4">
            <div className="space-y-1">
              ...
              <select 
                ...
                className="w-full px-3.5 py-2.5 bg-brand-50 border border-black/5 rounded-xl text-xs focus:outline-none"
              >
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                rows={4}
                required
                className="w-full px-3.5 py-2.5 bg-brand-50 border border-black/5 rounded-xl text-xs focus:outline-none resize-none"
              />
            </div>
            <button type="submit" className="w-full py-3 rounded-full bg-apple-dark hover:bg-brand-500 text-white text-xs font-bold uppercase tracking-wider transition-colors shadow-lg cursor-pointer">
              Submit review
            </button>
          </form>
        </div>
      ...
      <div className="border-t border-black/5 pt-16 space-y-6">
        <h3 className="font-display font-black text-xl text-apple-dark font-accent">Questions & Answers</h3>
        <div className="max-w-2xl space-y-4">
          {questions.map((qna, idx) => (
            <div key={idx} className="space-y-2 p-6 rounded-2xl bg-white border border-black/5">
          ...
          <form onSubmit={handleQuestionSubmit} className="flex gap-3 max-w-xl bg-white border border-black/5 p-1.5 rounded-2xl">
            <input
              type="text"
              value={newQuestion}
            ...
            />
            <button type="submit" className="px-4 py-2 rounded-xl bg-apple-dark hover:bg-brand-500 text-white text-xs font-semibold transition-colors">
              Ask
            </button>
          </form>
        </div>
